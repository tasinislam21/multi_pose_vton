...
class ConstantInput(nn.Module):
    def __init__(self, channel, size_y, size_x):
        super().__init__()
        self.input = nn.Parameter(torch.randn(1, channel, size_y, size_x))
...
class StyledConvBlock(nn.Module):
    def __init__(
        ...
        if initial:
            self.conv1 = ConstantInput(in_channel, size_y = 4, size_x = 3)
        ...
        self.conv2 = EqualConv2d(out_channel, out_channel, kernel_size, stride=1, padding=padding)
...
class StyledGenerator(nn.Module):
    def __init__(self, code_dim=512, n_mlp=8):
        ...
        for i in range(n_mlp):
            layers.append(EqualLinear(code_dim, code_dim)) #setting up mapping network
            layers.append(nn.LeakyReLU(0.2))

        self.style = nn.Sequential(*layers)

    def forward(self, input, step=0, alpha=-1, mean_style=None, style_weight=0,
        mixing_range=(-1, -1)):
        styles = []
        ...
        for i in range(step + 1):
            size_y = 4 * 2 ** i
            size_x = 3 * 2 ** i
            noise.append(torch.randn(batch, 1, size_y, size_x, device=input[0].device))
