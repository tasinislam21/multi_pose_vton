...
class KeyDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {'candidateHD': candidateHD, 'labelHD': labelHD, 'denseHD': denseHD, 'source_dense': source_denseHD,
                ...
                'source_dense_image': source_denseHD_image, 'warped_person': warped_person,
...
with torch.no_grad():
    gmm = xing.XingNetwork(input_nc=[3,3], output_nc=3)
    gmm.cuda()
    gmm.load_state_dict(torch.load('checkpoint/gmm_xing_49.pth'))
    gmm.eval()
...
for data in tqdm.tqdm(dataloader):
    ...
    fake_img = data['warped_person'].cuda()
    ...
    warped_garment, affine = gmm(input=[in_garmentHD, target_mask_clothes, in_skeletonHD])
