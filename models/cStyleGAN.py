...
class PoseEncoder(nn.Module):
    def __init__(self, ngf=64, blur_kernel=[1, 3, 3, 1], size=256):
        super().__init__()
        self.size = size
        convs = [ConvLayer(in_channel=3, out_channel=ngf, kernel_size=1)]
...
class SpatialAppearanceEncoder(nn.Module):
    ...
    def forward(self, input, pose, segment):
        coor = input[:,4:,:,:]
        input = input[:,:4,:,:]
        ...
        pose_mask = (pose[:,0, :, :] > 0).float().unsqueeze(1)
        pose_mask *= (1 - segment)
        ...
        p1 = torch.nn.functional.interpolate(pose_mask, size=(x1.shape[2], x1.shape[3]), mode='bilinear',
                                             align_corners=True)
        p2 = torch.nn.functional.interpolate(pose_mask, size=(x2.shape[2], x2.shape[3]), mode='bilinear',
                                             align_corners=True)
        p3 = torch.nn.functional.interpolate(pose_mask, size=(x3.shape[2], x3.shape[3]), mode='bilinear',
                                             align_corners=True)
        p4 = torch.nn.functional.interpolate(pose_mask, size=(x4.shape[2], x4.shape[3]), mode='bilinear',
                                             align_corners=True)
        p5 = torch.nn.functional.interpolate(pose_mask, size=(x5.shape[2], x5.shape[3]), mode='bilinear',
                                             align_corners=True)
        p6 = torch.nn.functional.interpolate(pose_mask, size=(x6.shape[2], x6.shape[3]), mode='bilinear',
                                             align_corners=True)
...
class GeneratorCustom(nn.Module):
    ...
    def forward(
        ...
        styles = self.appearance_encoder(appearance, target_dense, segment)
        ...
        out = self.pose_encoder(target_dense)
