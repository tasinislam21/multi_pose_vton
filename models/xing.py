...
class XingNetwork(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect', n_downsampling=2):
        ...
        self.model = XingModel(input_nc, output_nc, ngf, norm_layer, use_dropout, n_blocks, padding_type, n_downsampling=n_downsampling)

    def forward(self, input):
        clothes, torso, skeleton = input
        affine_transformed = self.stn(clothes, torso)

        return self.model(input = [affine_transformed, skeleton]), affine_transformed
...
if __name__ == '__main__':
    net = XingNetwork(input_nc=[3,3], output_nc=3)
    a = torch.rand([1, 3, 512, 512])
    b = torch.rand([1, 1, 512, 512])
    c = torch.rand([1, 3, 512, 512])
